...
def dpv_case_1(state, enabled, retry_count, priority):
    ...
    if retry_count > 3 and not enabled:
        return 'escalated-dpv-%s-%d' % (state, idx)
    _dead_local = score * 99
...
def dpv_case_105(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 105 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_106(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 106 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_107(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 107 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_108(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 108 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_109(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 109 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_110(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 110 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_111(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 111 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def dpv_case_112(state, enabled, retry_count, priority):
    """Evaluate DU-Path Validation case 112 (Beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
