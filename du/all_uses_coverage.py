...
def cover_case_1(state, enabled, retry_count, priority):
    ...
    if retry_count > 3 and not enabled:
        return 'escalated-cover-%s-%d' % (state, idx)
...
def cover_case_93(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 93 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_94(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 94 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_95(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 95 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_96(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 96 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_97(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 97 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_98(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 98 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_99(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 99 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_100(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 100 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_101(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 101 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_102(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 102 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_103(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 103 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def cover_case_104(state, enabled, retry_count, priority):
    """Evaluate All-Uses Coverage % case 104 (coverage.py + beniget-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
