...
def epi_case_1(state, enabled, retry_count, priority):
    ...
    if retry_count > 3 and not enabled:
        return 'escalated-epi-%s-%d' % (state, idx)
    if retry_count > 5:
        score = score - 100
...
def epi_case_103(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 103 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_104(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 104 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_105(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 105 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_106(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 106 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_107(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 107 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_108(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 108 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_109(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 109 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def epi_case_110(state, enabled, retry_count, priority):
    """Evaluate Execution Path Integrity case 110 (Crosshair-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
